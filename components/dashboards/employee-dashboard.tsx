...
// Check if work slot has started for today (after 8:00 AM)
const hasWorkSlotStartedToday = (): boolean => {
  const currentSlotIndex = getCurrentSlotIndexLagos()
  return currentSlotIndex >= WORK_SLOT_START
}
...
export default function EmployeeDashboard({ onLogout }: EmployeeDashboardProps) {
  ...
  const [workSlotStarted, setWorkSlotStarted] = useState<boolean>(false)
  ...
  useEffect(() => {
    ...
    // Check if work slot has started (after 8:00 AM)
    const started = hasWorkSlotStartedToday()
    setWorkSlotStarted(started)
  ...
  }, [isToday, timeSlots, workSlotEnded, workSlotStarted])
  ...
  const updateAvailableTimeSlots = () => {
    ...
    if (!started) {
      // Work hasn't started yet (before 8:00 AM) - no entries allowed
      setAvailableTimeSlots([])
    } else {
      // Work has started (after 8:00 AM) - show all work slots
  ...
  const handleLogActivity = async () => {
    ...
    // Check if work slot has started (after 8:00 AM)
    if (!workSlotStarted) {
      showToast.error("Work hasn't started yet. You can log activities from 8:00 AM onwards.")
      return
    }
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
              ...
              <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 flex-1">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 bg-gradient-to-br from-red-500 to-red-700 rounded-xl flex items-center justify-center shadow-lg shadow-red-300">
                ...
                <div className="flex flex-wrap items-center gap-3 shrink-0">
                  ...
                  <Dialog open={showCalendarModal} onOpenChange={setShowCalendarModal}>
                    ...
                    <DialogContent className="max-w-md p-0 border-0 shadow-xl rounded-2xl overflow-hidden">
                      <DialogHeader className="p-6 bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            {/* Professional Timer */}
            <DashboardTimer workSlotEnded={workSlotEnded} workSlotStarted={workSlotStarted} />
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Select 
                        ...
                        disabled={personalActivitiesLoading || !workSlotStarted}
                      >
                        <SelectTrigger className="w-full">
                          <SelectValue placeholder={
                            !workSlotStarted 
                              ? "Work hasn't started yet (8:00 AM)" 
                        ...
                        <SelectContent>
                          {availableTimeSlots.length > 0 ? (
                            ...
                            <div className="px-2 py-4 text-center text-sm text-slate-500">
                              ⏰ Work hasn't started yet. You can log activities from 8:00 AM onwards.
                            </div>
                          ) : (
                            <div className="px-2 py-4 text-center text-sm text-slate-500">
                    ...
                    <div>
                      ...
                      <Textarea
                        placeholder={!workSlotStarted ? "Work hasn't started yet" : "What are you working on?"}
                    ...
                    <Button
                      ...
                      disabled={!selectedTimeSlot || !activityDescription.trim() || personalActivitiesLoading || !workSlotStarted}
                    ...
                    {workSlotStarted && (
                      <div className="text-xs text-slate-500 p-3 bg-slate-50 rounded-lg border border-slate-200">
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            <span className="font-medium text-slate-700">Flexible Logging:</span> You can log activities for any time slot within today's work hours (8:00 AM - 5:30 PM), even if you forgot earlier. This helps capture all your work activities accurately.
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="border border-slate-200 rounded-lg">
                          ...
                          {isToday && (
                            <div className="flex items-center gap-2 text-xs">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-slate-600">Current slot: {currentSlotIndex >= WORK_SLOT_START && currentSlotIndex < WORK_SLOT_END ? slotIndexToTimeLabel(currentSlotIndex) : "Outside work hours"}</span>
                              </div>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="divide-y divide-slate-100">
                        {activityTable.map(entry => {
                          ...
                          const isWithinWorkHours = entry.slotIndex >= WORK_SLOT_START && entry.slotIndex < WORK_SLOT_END
                          
                          return (
                            <div 
                              key={entry.slotIndex} 
                              className={`p-4 transition-all duration-200 ${
                                isCurrentSlot ? 'bg-red-50/30 ring-1 ring-red-200/30' : 
                                isPastSlot ? 'bg-slate-50/50' : ''
                              } ${!isWithinWorkHours ? 'opacity-60' : ''}`}
                            >
                                ...
                                <div className="flex-1">
                                  ...
                                  <div className="flex items-center gap-2 mb-2">
                                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                                      entry.status === "present" ? "bg-green-100 text-green-800 border border-green-200" :
                                    ...
                                    {isCurrentSlot && isToday && isWithinWorkHours && (
                                      <span className="px-2 py-1 rounded text-xs bg-gradient-to-r from-red-100 to-red-50 text-red-700 font-medium animate-pulse flex items-center gap-1 border border-red-200">
                                        <div className="w-1.5 h-1.5 rounded-full bg-red-500"></div>
                                        Current Slot
                                      </span>
                                    )}
                                    {isFutureSlot && isToday && (
                                      <span className="px-2 py-1 rounded text-xs bg-blue-100 text-blue-700 font-medium border border-blue-200">
                                        Upcoming
                                      </span>
                                    )}
                                    {!isWithinWorkHours && (
                                      <span className="px-2 py-1 rounded text-xs bg-slate-100 text-slate-600 font-medium border border-slate-300">
                                        Off Hours
                                      </span>
                                    )}
                                  </div>
                                  ...
                                  {entry.activities.length > 0 ? (
                                    <div className="space-y-2">
                                      {entry.activities.map(activity => {
                                        ...
                                        return (
                                          <div key={activity.id} className="bg-white rounded-lg border border-slate-200 p-3 hover:shadow-sm transition-shadow duration-200">
                                            <div className="flex justify-between items-start">
                                              <div className="flex-1">
                                                <p className="text-sm font-medium text-slate-800 mb-1">{activity.description}</p>
                                                <div className="flex items-center gap-2">
                                                  <span className={`text-xs px-2 py-1 rounded-full ${statusColor} border`}>
                                                    {activity.status}
                                                  </span>
                                                </div>
                                              </div>
                                              {isToday && workSlotStarted && (
                                    ...
                                    isPastSlot && isToday && (
                                      <div className="text-sm text-slate-400 italic bg-slate-50 p-3 rounded-lg border border-slate-100">
                                        No activity logged for this time slot
                                      </div>
                                    )
                                  )}
                                </div>
